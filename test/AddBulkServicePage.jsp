...
<%
	try {
		List<String> strList = (List<String>) request.getSession().getAttribute("strList");
		System.out.println("List Size : " + strList.size());
%>

<head>
<meta http-equiv="X-UA-Compatible" content="IE=8; IE=9; IE=10; IE=EDGE; chrome=1"></head>
<body>
...
function moveOptionsProfiles(option){                  
     ...
     // alert(clauseOptionsSelected.value);
       ...
       clauseOptionsSelected.options[0].selected= true;
       //alert(clauseOptionsSelected.options[clauseOptionsSelected.selectedIndex].value);
}
...
function sendReqParameters(){
   // alert("Iamhere");
    ...
    %>
    if(tempp==""){
        tempp = tempp+document.getElementById("min<%=st%>Range").value;
    }else{
        tempp = tempp+"#"+document.getElementById("min<%=st%>Range").value;
    }
    
    <%}%>
    document.getElementById("reqParameters").value = tempp;
    //alert(tempp);
		...
		<s:form action="AddBulkResponseAction.action" name="GeneratedXpathsForm" method="post" enctype="multipart/form-data">
			<table align="center" width="600" class=menu1 border="1">
				...
				<tr>
					...
					<td><input type="button" name="button5" size="5" value="&nbsp;&nbsp;&nbsp;&gt;&nbsp;&nbsp;&nbsp;" onclick="javscript:moveOptionsProfiles('selectedToRightProfiles')" />
						<br><input type="button" value="&nbsp;&nbsp;&nbsp;&lt;&nbsp;&nbsp;&nbsp;" name="button7" size="5" onclick="javascript:moveOptionsProfiles('selectedToLeftProfiles')">
						&nbsp; &nbsp; &nbsp;
					</td>
					<td>
						<select name="SelectedXpaths" id="SelectedXpaths" key="SelectedXpaths" SIZE="+10" style="width: 70mm" multiple="multiple" onchange="valueChange(this.value);"></select>
					</td>
...
<td><input type="text" name="min<%=st%>Range" value="000" id="min<%=st%>Range"/></td>

<td><input type="text" name="min<%=st%>Range" value="000" id="max<%=st%>Range"/></td>

</tr>

				<%}%>
				<tr align="right">
					<td><button type="button" name="Generate XMLs" onclick="sendReqParameters();">Generate XMLs</button></td>
				</tr>
			</table>

			<s:hidden id="selectedXpathsToAction" value="" key="serviceInfo.selectedXpaths" />
			<s:hidden id="elementToBeReplaced" name="elementToBeReplaced" value=""></s:hidden>
			<s:hidden id="reqParameters" name="reqParameters" value=""/>
