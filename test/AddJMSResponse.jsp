...
function addOrEditSelection(){
	 ...
	 if(responseKey=="Add New Response")
		 ...
		 var dropValues="";
		 var dropObj = document.getElementById("key");
		 for(var i =0; i<dropObj.options.length;i++){
			 
		 }
		 //obj.action='ShowAddNewResponsePageAction.action';
		 obj.action='addNewJMSResponse.action';
	     obj.submit();	
		 }
	 else
		 {
		 //obj.action='ShowEditResponsePageAction.action';
		 obj.action='editJMSResponse.action';
		 obj.submit();
		 }
}
...
<tr><td><s:select label="Operation Name : " id="subSystem" key="operName" list="{'Select from below'}" disabled="true" onchange="showExistingResponses()"/></td></tr>
<tr><td><s:select label="Response Identifier : " id="key" key="respIdentifierKey" list="{'Select from below'}" disabled="true" onchange="addOrEditSelection()" /></td></tr>
